...
export async function POST(req: NextRequest) {
  ...
  const arefCol = headers.findIndex((h) => h.includes('aref'));
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const codeAref = arefCol >= 0 ? cols[arefCol] ?? '' : '';
    ...
    created.push({ email, full_name: fullName, codeAref, verificationCode: randomVerificationCode() });
  }

  return Response.json({
    message: `${created.length} RESPONSABLE_SI created, ${skipped.length} skipped`,
    created,
    skipped,
  }, { status: 201 });
}
