    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto', py: 2 }}>
      ...
      <window.ReviewTable
        ...
        onRowUpdate={(id, field, val) => {
          ...
          const revalidated = window.validateAllRows(updated);
        ...
        {phase === 'review' && (
          <Stack direction="row" spacing={1.5}>
            <Button
              ...
              sx={{ minWidth: 180 }}
            >
              {importMode === 'dryrun' ? '🔍 Run Dry Run' : importMode === 'valid_only' ? `✅ Import ${validRows.length} Valid Rows` : `🚀 Import All ${importedRows.length} Rows`}
            </Button>
          </Stack>
        )}
