    ...
    <Box sx={{ maxWidth: 720, mx: 'auto', py: 2 }}>
      ...
      <Paper variant="outlined" sx={{ p: 3, mb: 2, borderRadius: 2 }}>
        ...
        {config.auth?.type === 'oauth2' && (
          <Stack spacing={2}>
            <TextField label="Token URL" fullWidth size="small" value={config.auth?.tokenUrl || ''} onChange={e => handleAuthChange('tokenUrl', e.target.value)} placeholder="https://auth.yourapp.com/oauth/token" />
          ...
          {endpoints.map((ep) => (
            <Box key={ep.id} sx={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr auto', gap: 1, alignItems: 'center' }}>
              ...
              <TextField
                ...
                InputProps={{
                  startAdornment: <InputAdornment position="start"><Typography variant="caption" color="text.secondary">/</Typography></InputAdornment>
                }}
              />
