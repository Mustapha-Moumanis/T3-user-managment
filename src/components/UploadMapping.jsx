          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 2, borderRadius: 2 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 1.5, alignItems: 'center' }}>
              ...
              {rawData.headers.map(header => {
                ...
                const field = CANONICAL_FIELDS.find(f => f.key === mappedTo);
                return (
          ...
          <Paper variant="outlined" sx={{ mb: 3, borderRadius: 2, overflow: 'hidden' }}>
            ...
            <Box sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    {Object.keys(mapping).filter(k => mapping[k] && mapping[k] !== '_skip').map(field => (
                      <TableCell key={field}>{CANONICAL_FIELDS.find(f => f.key === field)?.label || field}</TableCell>
                    ))}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {previewRows.map((row, i) => {
                    const mapped = window.applyMapping([row], mapping)[0];
                    return (
                      <TableRow key={i}>
                        {Object.keys(mapping).filter(k => mapping[k] && mapping[k] !== '_skip').map(field => (
                          <TableCell key={field}>
                            <Typography variant="caption">{mapped[field] || <span style={{ opacity: 0.3 }}>—</span>}</Typography>
                          </TableCell>
                        ))}
                      </TableRow>
                    );
...
Object.assign(window, { UploadMapping, CANONICAL_FIELDS });
