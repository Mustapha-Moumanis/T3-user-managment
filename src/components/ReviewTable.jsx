      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5, flexWrap: 'wrap' }}>
        <Stack direction="row" spacing={0.5}>
          {[
            ...
            ...(showImportStatus ? [
              { val: 'success', label: `Imported`, color: 'success' },
              { val: 'failed', label: `Failed`, color: 'error' },
            ] : []),
          ].map(f => (
            <Chip
              key={f.val}
              ...
              {fields.map(field => (
                <TableCell key={field} onClick={() => handleSort(field)} sx={{ cursor: 'pointer', userSelect: 'none', whiteSpace: 'nowrap' }}>
                  <Stack direction="row" alignItems="center" spacing={0.5}>
                    <Typography variant="caption" fontWeight={700}>{FIELD_LABELS[field] || field}</Typography>
                    {sortField === field && (
...
Object.assign(window, { ReviewTable, FIELD_LABELS });
